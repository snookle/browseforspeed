...
public class faq {
	public static HTML getHTML () {
		...
		answers.add("Browse for Speed is a server browser for the popular online racing game <a href='http://www.liveforspeed.net'>Live for Speed</a>. It is able get a list of servers from the LFS master server, show some extended information about a server (players currently in the server). It also lets you save your favourite servers.");
		questions.add("How can I make it faster? What is this 'Disable query wait' checkbox and value? All servers suddenly start giving 'refused connection'!");
		answers.add("Windows XP SP2 introduced a limit of 10 outgoing TCP connections per second (this is completely ridiculous). If your OS does not have this limit (XP SP2 definitely does have this limit), you may be able to check this box, and the whole query process will be much, much faster. If you check this when you shouldn't, queries will start failing after a random amount of succeeded queries (Bill seems unable to test this limit very accurately. See the following page for more information, and a possibly way to remove this limit on XPSP2: <a href='http://www.speedguide.net/read_articles.php?id=1497'>http://www.speedguide.net/read_articles.php?id=1497</a>).<br/><br/>If, when refreshing, the servers all start giving 'refused connection', increase the value of the 'Query wait value'. This is the number of milliseconds to wait before adding another server to the list of concurrently queried servers.");
